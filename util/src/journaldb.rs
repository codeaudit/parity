...
impl JournalDB {
	...
	fn commit_with_overlay(&mut self, now: u64, id: &H256, end: Option<(u64, H256)>) -> Result<u32, UtilError> {
		...
		if let Some((end_era, canon_id)) = end {
			...
			let mut canon_removes: Vec<H256> = Vec::new();
			while journal_overlay.journal.front().map_or(false, |e| e.era <= end_era) {
				let journal = journal_overlay.journal.pop_front().unwrap();
				...
				{
					let to_remove = if canon_id == journal.id { &journal.deletions } else { &journal.insertions };
					for k in to_remove {
				...
				if canon_id == journal.id {
					canon_inserts = journal.insertions;
					canon_removes = journal.deletions;
				}
			}
			// apply canon to backing database
			for k in canon_inserts {
				if let Some(v) = journal_overlay.backing_overlay.lookup(&k) {
					try!(batch.put(&k, &v));
				}
			}
			for k in canon_removes {
		...
		if let Some(val) = db.get(&LATEST_ERA_KEY).expect("Low-level database error.") {
			...
			loop {
				...
				while let Some(rlp_data) = db.get({
					...
					let to_add: Vec<Bytes> = rlp.val_at(1);
					let to_remove: Vec<H256> = rlp.val_at(2);
					let mut inserted_keys = Vec::with_capacity(to_add.len());
					for v in to_add {
						let k = overlay.insert(&v);
